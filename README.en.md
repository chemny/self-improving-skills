...
Self-Improving Skills is a single-skill package for agents that need to improve over time without depending on a pile of manual notes or fragile prompt tweaks.
...
Self-Improving Skills gives the agent a structured way to decide:
...
Self-Improving Skills is inspired by Andrej Karpathy's Software 3.0 / LLM OS framing and the broader context engineering discussion: LLM behavior is increasingly shaped by natural-language instructions, context, tools, memory, examples, feedback, evaluation, and pruning, not only by traditional code.
...
Self-Improving Skills turns that idea into a small, operational skill:
...
Project Ledger is Self-Improving Skills' project outcome layer inside scan reports. Before memory triage, it answers:
...
This keeps Self-Improving Skills from becoming only a keyword scanner. It first makes real project outcomes visible, then decides what deserves memory.
...
When installed in Codex, Self-Improving Skills can create a 6-hour graded scan automation:
...
Self-Improving Skills does not treat all learnings equally.
...
Self-Improving Skills uses three memory files:
...
Self-Improving Skills is a single-skill repository with a lightweight installer.
...
Self-Improving Skills will not automatically:
